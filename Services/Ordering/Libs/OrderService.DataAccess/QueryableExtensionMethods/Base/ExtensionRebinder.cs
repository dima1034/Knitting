...
namespace OrderService.DataAccess.QueryableExtensionMethods.Base
{
    internal class ExtensionRebinder : ExpressionVisitor
    {
        ...
        public ExtensionRebinder(
            object originalQueryable,
            Expression replacementQueryable,
        ...
        {
            return node.Value == _originalQueryable
                ? _replacementQueryable
                : node;
        }
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            if (node.NodeType == ExpressionType.MemberAccess
                ...
                && node.Expression.Type.GetTypeInfo()
                       .IsDefined(typeof(CompilerGeneratedAttribute)))
            {
                string argumentName = node.Member.Name;

                Expression replacement = _argumentReplacements.Where(p => p.Key == argumentName)
                                                              .Select(p => p.Value)
                                                              .FirstOrDefault();
